...
fun score_submission_inserts_and_sorts_correctly() {
    ...
    lb.submit_score_for_testing(ALICE, 50, 111, 1000);
    lb.submit_score_for_testing(BOB, 100, 222, 2000);
    lb.submit_score_for_testing(CAROL, 75, 333, 3000);
...
fun full_leaderboard_rejects_low_score() {
    ...
    lb.submit_score_for_testing(ALICE, 50, 111, 1000);
    lb.submit_score_for_testing(BOB, 40, 222, 2000);
    lb.submit_score_for_testing(CAROL, 30, 333, 3000);
...
#[test, expected_failure(abort_code = leaderboard::EScoreNotQualified)]
fun submit_below_threshold_aborts() {
    let mut ctx = tx_context::dummy();
    let mut lb = leaderboard::create_for_testing(3, &mut ctx);
    ...
    // This should abort — score 20 doesn't qualify
    lb.submit_score_for_testing(DAVE, 20, 444, 4000);
...
fun full_leaderboard_accepts_higher_score_and_trims() {
    ...
    lb.submit_score_for_testing(DAVE, 45, 444, 4000);
...
fun tie_breaking_prefers_earlier_timestamp() {
    ...
    lb.submit_score_for_testing(BOB, 50, 222, 2000);
...
fun tie_at_threshold_prefers_earlier_timestamp() {
    ...
    lb.submit_score_for_testing(ALICE, 100, 111, 1000);
    lb.submit_score_for_testing(BOB, 50, 222, 2000);
    lb.submit_score_for_testing(CAROL, 30, 333, 3000); // last place, score 30 at time 3000

    // DAVE scores 30 at time 2500 (earlier than CAROL) — should qualify via tie-break
    assert!(lb.qualifies(30, 2500));
    lb.submit_score_for_testing(DAVE, 30, 444, 2500);
